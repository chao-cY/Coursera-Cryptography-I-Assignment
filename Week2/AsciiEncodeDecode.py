#本文件用于将16进制密文还原到其对应的十进制ASCII码
import binascii
import codecs
import sys


##要想解码，需先将十六进制转为二进制，才可解码至字符
#str_bin = binascii.unhexlify(s_hex) 
#print( binascii.unhexlify(s).decode(errors="replace"))

def getAaciiHex(str):
    return str.encode().hex()

def getStrFromHex(hex):
    return binascii.unhexlify(hex).decode(errors='replace')

#字符与十进制ASCII编码转换的方法
def getAscii():
    #65A-90Z   ASCII十进制编码构成的数组
    ...
    #从下面测试可看出，与space32进行异或运算得到的字符依然是字母
    print("".join([chr(c ^ 32) for c in msg]))  
    print("".join(chr(c) for c in msg))

    #但如果是两个字母进行异或运算，得到的字符很可能超出字母编码范围
    print(chr(ord('c') ^ ord('s')))

    #因此，如果两个字符异或后得到的编码仍然在字母编号内，则说明其中一个字符很可能是space32
