...
def main():
	...
	# 将hex格式密文依据ASCII转换为字符密文
	#cypherTextsDecoded = map(methodcaller("decode", "hex"), cypherTexts) #在python3中不可用
	cypherTextsDecoded = [(binascii.unhexlify(ct1).decode(errors='replace')) for ct1 in cypherTexts]


	#选择两个密文进行异或，遍历得到的字符串，如果字符是字母，则说明其中一个明文是空格32，另一个明文为当前字符 xor 32 得到的字符
	...
	xoredText = strxor(cypherTextsDecoded[xorTextNum1], cypherTextsDecoded[xorTextNum2])  #两个密文异或结果等于两个明文异或

	# loop all individual positions of the XOR-ed crypto (plain) text
	textLength = len(xoredText)
	for i in range(textLength): #遍历m6 xor m10 运算后得到的字符串
		# XOR the substring with the crib
		substrXoredText = xoredText[i:]
		cribDraggedText = strxor(substrXoredText,crib)  #没看懂？？？？
...
#传入参数当前下标，m1 xor m2的子串xor crib，m1 xor m2的长度，‘.’
