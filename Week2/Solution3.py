

# coding=utf-8
import codecs
 
def main():
    ciphertext=[0]*11  #创建一个有11个元素的数组
 
    ciphertext[0]="32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
 
    ciphertext[1]=""
 
    ciphertext[2]="234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
 
    ciphertext[3]="32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
 
    ciphertext[4]=""
 
    ciphertext[5]="3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
 
    ciphertext[6]=""
 
    ciphertext[7]=""
 
    ciphertext[8]="315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3"
 
    ciphertext[9]=""
 
    ciphertext[10]="466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
 
    cipher=[]
    for i in range(11):
        cipher.append(''.join([chr(x) for x in split_hex(ciphertext[i])]))   #调用split_hex将16进制密文转为10进制，随后调用chr,得到16进制密文对应的密文字符
 
    for j in range(1,11):
        result=[]
        for i in range(1,11):
            if i==j:   #选中相同密文则跳过
                continue
            raw=strxor(cipher[j], cipher[i])  #若选择的密文不同，则对两个密文进行异或操作
            raw=list(raw) #字符串转list
            for i,letter in enumerate(raw):
                letter=ord(letter) #获得单个字符的ASCII十进制
                if (letter>=0x41 and letter<=0x5a) or (letter>=0x61 and letter<=0x7a): #如果是字母字符，则跳过
                    pass
                else:
                    raw[i]='_' #如果不是字母字符，则将字符替换为‘_’
            result.append(''.join(raw)) #更新后的raw，添加到result数组
            print(''.join(raw))
 
        length=len(cipher[j])//2    #密文长度的一半
 
        plain=['=']*length   #创建一个有length个‘=’的数组
        for i in range(length):
            count_=0
            countL=0
            mark=''
            only=True
            for letter in [string[i] for string in result]:
                if letter=='_':
                    count_+=1
                else:
                    countL += 1
                    if mark!='' and mark == letter.lower():
                        pass
                    elif mark!='' and mark != letter.lower():
                        only=False
                    mark=letter.lower()
 
            if countL==1:
                plain[i]=mark
            elif countL>1:
                if only:
                    plain[i] = mark
            else:
                plain[i]='#'
        print()
        print(''.join(plain))
 
        plaintext="The nice thing about Keeyloq is now we cryptographers can drive a lot of fancy cars - Dan Bone"
        print(strxor(strxor(plaintext,cipher[3]),cipher[6]))
 
 
 
def list2hex(lo):
    h=["{:02x}".format(i) for i in lo]
    h="".join(h)
    return h
 
#十六进制转十进制
def split_hex(h, dtype='int'):
    '''
    :stuck_out_tongue_winking_eye:aram h: A hex string. Example: "315c4eea". Its length should be an even number.
    :return: A list of integer corresponding to every two letters in h
    '''
    if dtype == 'int':
        split = [int(h[2 * i:2 * i + 2], 16) for i in range(len(h) // 2)]
    elif dtype == 'str' or 'string':
        split = [h[2 * i:2 * i + 2] for i in range(len(h) // 2)]
    else:
        split = [int(h[2 * i:2 * i + 2], 16) for i in range(len(h) // 2)] 
    return split
 
def strxor(a, b):     # xor two strings of different lengths
    if len(a) > len(b):
       return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
       return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])
 
def encrypt(key, msg):
    c = strxor(key, msg)
    c=codecs.encode(bytes(c,encoding='utf-8'), 'hex')
    return c
 
def decrypt(key, cipher):
    cipher=''.join([chr(x) for x in split_hex(cipher)])
    msg=strxor(key, cipher)
    return msg
 
 
if __name__=="__main__":
    main()
